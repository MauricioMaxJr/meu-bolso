#!/usr/bin/env node
/* GERAR HTML ÚNICO - empacota o MAXBOLSO num arquivo só (maxbolso.html na
...
for (const f of ["index.html", "kit/maxworks-ui.css", "kit/maxworks-ui.js", "app.css", "app.js", "icons.js", "holerite.js", "extrato.js"])
...
html = html.replace(/<link rel="stylesheet" href="kit\/maxworks-ui\.css[^"]*">/,
  () => `<style>\n${ler("kit/maxworks-ui.css")}\n</style>`);
html = html.replace(/<link rel="stylesheet" href="app\.css[^"]*">/, () => `<style>\n${css}\n</style>`);

// manifest e ícones de instalação não fazem sentido em arquivo local
html = html.replace(/^.*rel="manifest".*\r?\n/m, "");
html = html.replace(/^.*rel="apple-touch-icon".*\r?\n/m, "");
html = html.replace(/^.*rel="icon".*\r?\n/m, "");

// scripts embutidos na mesma ordem do index (</script> em string vira <\/script>)
const antiFecho = s => s.replace(/<\/script/gi, "<\\/script");
html = html.replace(/<script src="kit\/maxworks-ui\.js[^"]*"><\/script>/,
  () => `<script>\n${antiFecho(ler("kit/maxworks-ui.js"))}\n</script>`);
...
const SAIDA = join(RAIZ, "..", "maxbolso.html");
